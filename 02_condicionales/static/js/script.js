...
function calificacionChile() {
    let nota = parseFloat(prompt("Ingrese nota: ")); // "parseFloat" recibe numeros decimales y enteros
    if (nota >= 6 && nota <= 7) { // "&&" dos comparasiones 
        alert("Excelente, eximido")
    } else if (nota >= 4 && nota <= 5.9) { // ">=" mayor o igual que, "<=" menor o igual que 
        alert("Aprobado, vas a examen")
    } else if (nota < 4 && nota >= 1) { //"<" menor que, ">" mayor que
        alert("Reprobado")
    } else {
...
function ejercicio10() {
    ...
    nombreVisita.trim(); // Elimina los espacios del inicio y final
    if (nombreVisita == "") { // Verifica que el nombre no esté vacio
        alert("Error: el nombre no puede estar vacio.")
    } else if (bitacora.includes(nombreVisita)) { // Verifica que el nombre esté repetido
